...
MainWindow::MainWindow(QWidget *parent) : QWidget(parent), mIsReady(false), mHasPendingPlay(false), mControlOn(false), mShowControl(2), mRepeateMax(0), mpVOAction(0), mpStatisticsView(0)
{
...
void MainWindow::initPlayer()
{
    ...
    _widgetVideo_2 = new VlcWidgetVideo(this);
    _widgetVideo = new VlcWidgetVideo(this);
    _player->setVideoWidget(_widgetVideo);
    _widgetVideo->setMediaPlayer(_player);


    //mpPlayer->setAudioOutput(AudioOutputFactory::create(AudioOutputId_OpenAL));
    EventFilter *ef = new EventFilter(_player);
    ...
    connect(_player, SIGNAL(playing()), this, SLOT(onStartPlay()));
    connect(_player, SIGNAL(stopped()), this, SLOT(onStopPlay()));
    connect(_player, SIGNAL(paused()), this, SLOT(onPaused()));
    //connect(mpPlayer, SIGNAL(speedChanged(qreal)), this, SLOT(onSpeedChange(qreal)));
    connect(_player, SIGNAL(timeChanged(int)), this, SLOT(onPositionChange(int)));
    ...
    mpPlayerLayout->addWidget(_widgetVideo);
...
{
    qDebug("seek finished at %lld/%lld", pos, _player->position());
}
...
void MainWindow::play(const QString &name)
{
    ...
    _player->stop(); //if no stop, in setPriority decoder will reopen
    ...
    _media.reset(new VlcMedia(name,true));
    _player->open(_media.get());
}
...
{
    if (_player->state() == Vlc::Playing || _player->state() == Vlc::Paused) {
        _player->togglePause();
    } else {
        if (mFile.isEmpty())
            return;
...
void MainWindow::onSpinBoxChanged(double v)
{
    if (!_player)
        return;
    _player->setPlaybackRate(v);
}

void MainWindow::onPaused()
{
    auto p = _player->state() == Vlc::Paused;
    if (p) {
...
void MainWindow::onStartPlay()
{
    ...
    mFile = _player->currentMedia()->currentLocation(); //open from EventFilter's menu
    mTitle = mFile;
    if (!mFile.contains(QLatin1String("://")) || mFile.startsWith(QLatin1String("file://")))
        mTitle = QFileInfo(mFile).fileName();
    setWindowTitle(mTitle);
    ...
    mpPlayPauseBtn->setIcon(QIcon(QString::fromLatin1(":/theme/dark/pause.svg")));
    mpTimeSlider->setMinimum(0);
    mpTimeSlider->setMaximum(_player->length());
    mpTimeSlider->setValue(0);
    mpTimeSlider->setEnabled(_player->seekable());
    mpEnd->setText(QTime(0, 0, 0).addMSecs(_player->length()).toString(QString::fromLatin1("HH:mm:ss")));
    setVolume();
    mShowControl = 0;
    QTimer::singleShot(3000, this, SLOT(tryHideControlBar()));
    mpPowerManagement->setActivityState(true);
    initAudioTrackMenu();
    //mpRepeatA->setMinimumTime(QTime(0, 0, 0).addMSecs(_player->mediaStartPosition()));
    //mpRepeatA->setMaximumTime(QTime(0, 0, 0).addMSecs(_player->mediaStopPosition()));
    //mpRepeatB->setMinimumTime(QTime(0, 0, 0).addMSecs(_player->mediaStartPosition()));
    //mpRepeatB->setMaximumTime(QTime(0, 0, 0).addMSecs(_player->mediaStopPosition()));
    //mpRepeatA->setTime(QTime(0, 0, 0).addMSecs(_player->startPosition()));
    //mpRepeatB->setTime(QTime(0, 0, 0).addMSecs(_player->stopPosition()));
    mCursorTimer = startTimer(3000);
    PlayListItem item = mpHistory->itemAt(0);
    item.setUrl(mFile);
    item.setTitle(mTitle);
    item.setDuration(_player->time());
...
void MainWindow::onPositionChange(int pos)
{
    if (_player->seekable())
