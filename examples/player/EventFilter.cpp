...
EventFilter::EventFilter(VlcMediaPlayer *player) : QObject(player),
                                                   menu(0)
{
}
...
void EventFilter::openLocalFile()
{
    ...
    if (file.isEmpty())
        return;
    VlcMediaPlayer *player = static_cast<VlcMediaPlayer *>(parent());
    player->stop();
    _media.reset(new VlcMedia(file, true, this));
    player->open(_media.get());
}
...
void EventFilter::openUrl()
{
    ...
    VlcMediaPlayer *player = static_cast<VlcMediaPlayer *>(parent());
    player->stop();
    _media.reset(new VlcMedia(url, this));
    player->open(_media.get());
}
...
void EventFilter::help()
{
    ...
    static QString help = QString::fromLatin1("<h4>") + tr("Drag and drop a file to player\n") + QString::fromLatin1("</h4>"
                                                                                                                     "<p>")
                          + tr("A: switch aspect ratio") + QString::fromLatin1("</p>"
                                                                               "<p>")
                          + tr("Double click to switch fullscreen") + QString::fromLatin1("</p>"
                                                                                          "<p>")
                          ...
                          + tr("S: stop\n") + QString::fromLatin1("</p>"
                                                                  "<p>")
                          + tr("R: rotate 90") + QString::fromLatin1("</p>"
                                                                     "<p>")
                          + tr("M: mute on/off\n") + QString::fromLatin1("</p>"
                                                                         "<p>")
                          + tr("C: capture video") + QString::fromLatin1("</p>"
                                                                         "<p>")
                          + tr("Up/Down: volume +/-\n") + QString::fromLatin1("</p>"
                                                                              "<p>")
...
{
    Q_UNUSED(watched);
    VlcMediaPlayer *player = qobject_cast<VlcMediaPlayer *>(parent());
    ...
    case QEvent::Drop: {
        ...
        if (e->mimeData()->hasUrls()) {
            ...
            player->stop();
            _media.reset(new VlcMedia(path, true, this));
            player->open(_media.get());
            e->acceptProposedAction();
        } else {
            e->ignore();
...
WindowEventFilter::WindowEventFilter(QWidget *window)
    : QObject(window), mpWindow(window)
{
}
